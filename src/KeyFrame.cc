...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mvuRight(F.mvuRight), mvDepth(F.mvDepth), mDescriptors(F.mDescriptors.clone()), mPreGlobalDescriptors(F.mPreGlobalDescriptors.clone()), mGlobalDescriptors(F.mGlobalDescriptors.clone()),
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false), imgLeft(F.imgLeft) // For Debug
{
    mnId=nNextId++;
